...
class ReservaCancelIntegrationTest {
    ...
    void cancelarReservaDeveRedirecionarParaLista() throws Exception {
        assertMocksInjected();

        mockMvc.perform(post("/reservas/42/cancelar").with(csrf()))
            .andExpect(status().is3xxRedirection())
            .andExpect(redirectedUrl("/reservas"));

        verify(reservaService).cancelar(42L);
    }
