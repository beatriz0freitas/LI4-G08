...
2. Implementação backend inicial
  - Implementar `RelatorioService` — agregações (ocupacaoPerc, estadiasCount, reservasCount, faturacaoTotal, servicosExtraTotal)
  - Criar queries em `PagamentoRepository` para sumarização por período/método
  - Implementar `ColaboradorController`, `ColaboradorService`, `ColaboradorRepository` (CRUD + desactivar)
...
3. Implementar `RelatorioService` e testes básicos
